...
import os, json, random, torch, transformers
...
class DataCollatorForSupervisedDataset:
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        text_input, data_type = tuple([instance[key] for instance in instances] for key in ("text_input", "data_type"))
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments) -> Dict:
    ...
    dataset = Mix_dataset(
        train_json, data_args.batch_size, img_size=data_args.img_size, hd_num=data_args.hd_num, local_rank=local_rank
    )
    print(str(len(dataset)) + "samples is loaded")

    train_dataset, eval_dataset = random_split(
        dataset,
        [data_args.train_test_split, 1 - data_args.train_test_split],
        torch.Generator().manual_seed(data_args.train_test_seed),
    )
...
def parse_args() -> tuple[ModelArguments, DataArguments, TrainingArguments, LoraArguments]:
    global local_rank
