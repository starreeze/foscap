...
class Mix_dataset(Dataset):
    ...
    def __init__(self, json_datas, batch_size=1, img_size=224, local_rank=0, hd_num=-1):
        ...
        for _, d in json_datas.items():
            if "image" in d[0].keys():
                ...
                has_img = False
            sub_data_set = Sample_dataset(d, batch_size, has_img=has_img, img_size=img_size, hd_num=hd_num)
            if has_img:
        ...
        self.data_ratio_multi = [float(ratio) / sum(self.data_num_multi) for ratio in self.data_num_multi]
        self.data_ratio_text = [float(ratio) / sum(self.data_num_text) for ratio in self.data_num_text]
    ...
    def __getitem__(self, index):
        ...
        if len(self.datasets_multi) == 0 and len(self.datasets_text) == 0:
            raise ValueError("Both _multi and _text are empty. Cannot sample any data.")

        if len(self.datasets_multi) > 0 and (self.use_multi < self.batch_size or len(self.datasets_text) == 0):
            data_idx = random.choices(range(len(self.data_ratio_multi)), weights=self.data_ratio_multi, k=1)[0]
            sample = self.datasets_multi[data_idx].get_item()
        elif len(self.datasets_text) > 0:
            data_idx = random.choices(range(len(self.data_ratio_text)), weights=self.data_ratio_text, k=1)[0]
...
class Sample_dataset(Dataset):
    ...
    def __init__(self, raw_data, batch_size, has_img=True, img_size=224, hd_num=16):
        ...
        if hd_num == -1:
            ...
            # for 4khd model
            self.vis_processor = ImageProcessorHD(image_size=img_size, hd_num=hd_num)
